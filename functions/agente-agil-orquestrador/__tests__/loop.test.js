...
test('buildTools() expõe o vocabulário de outputs do Agente Ágil + perguntar_humano + ler_card + visao_board + biblioteca_agil + criar_card + cards_por_agente + notificar_especialista_externo', () => {
  const tools = buildTools();
  const names = tools.map((t) => t.name);

  assert.deepEqual(
    names.sort(),
    ['agent_status', 'biblioteca_agil', 'cards_por_agente', 'checklist_item', 'comentario', 'criar_card', 'editar_campos', 'ler_card', 'link', 'mover_coluna', 'notificar_especialista_externo', 'perguntar_humano', 'relatorio_html', 'risco', 'visao_board'].sort(),
  );
  tools.forEach((t) => {
