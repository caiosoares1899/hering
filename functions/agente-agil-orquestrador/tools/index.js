...
function buildTools(options = {}) {
  ...
  tools.push({
    ...
    description:
      ...
      mode === 'real'
        ? `Cria um card NOVO (rascunho revisável por um humano — não entra direto no board) quando a informação recebida não é sobre nenhum card existente. Use titulo obrigatório; descricao/prazo/squad_solicitante/checklist/tags/riscos opcionais; submarca só é exigida se o squad tiver Submarca ativa (a ferramenta avisa as opções válidas se faltar ou vier errada). checklist é uma lista de até 20 textos curtos — use quando a informação recebida já descrever um processo/passo a passo, em vez de só narrar as etapas dentro da descrição (os itens nascem desmarcados). tags é uma lista de até 10 NOMES de tag (não IDs — a ferramenta casa por nome ao confirmar; nomes sem correspondência no squad são ignorados em silêncio). riscos é uma lista de até 10 textos curtos, um por risco identificado. Se o squad exigir Ficha Técnica, a ferramenta recusa — peça pra um humano criar manualmente nesse caso. ${dryRun ? 'Monta o que faria, mas em dryRun — NÃO cria nenhum rascunho de verdade; se chamar essa ferramenta em dryRun, seu comentário final precisa deixar claro que nada foi criado ainda, nunca afirmar que o rascunho existe.' : 'Cria o rascunho DE VERDADE em Pedidos de Intake.'}`
